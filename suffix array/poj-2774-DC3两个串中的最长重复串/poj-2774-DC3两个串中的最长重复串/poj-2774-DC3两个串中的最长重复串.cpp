// poj-2774-DC3两个串中的最长重复串.cpp : 定义控制台应用程序的入口点。
//
...
void dc3(int* r, int* sa, int n, int m)
{
	...
	sort(r,wa,wb,tbc,m);//wb[i]为前三个字符排名下排名i的起始索引
	for(p=1,rn[F(wb[0])]=0,i=1;i<tbc;i++)//按排名顺序  依次将起始索引转为F后计算相应rank
...
{  
    ...
    for (i=3;i<=n;i++)  
        if (a[i]!=a[i-1] && height[i]>ans)  //不在一个串中  取height最大值
           ans=height[i];  
    return ans;  
} 
int main()
